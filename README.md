<p align="center">
  <img src="./assets/profile-banner-art.svg" alt="Profile banner" width="100%" />
</p>

<h1 align="center">Hi, I'm Công</h1>

<h3 align="center">Software Engineer</h3>

<p align="center">
  Backend · Full-stack · System Design · AI
</p>

<p align="center">
  Building reliable software from <strong>requirements → business rules → system design → implementation</strong>.
</p>

---

<h2 align="center">About Me</h2>

<table align="center" border="0">
  <tr>
    <td width="62%" valign="middle">
      <p>
        I'm a final-year Software Engineering student at <strong>FPT University</strong>, interested in building systems where engineering, business rules, and user needs have to work together.
      </p>
      <p>
        I work mainly with <strong>Java / Spring Boot</strong> and modern full-stack technologies, while also exploring AI-powered applications and RAG workflows.
      </p>
      <p>
        I enjoy taking ownership of a system beyond coding — understanding requirements, designing workflows and data models, implementing the core features, and working with a team to bring the system together.
      </p>
      <p>
        <strong>Long-term goal:</strong> grow into a Development Lead who can connect business requirements, system design, and engineering execution.
      </p>
    </td>
    <td width="38%" align="center" valign="middle">
      <img src="./assets/profile-character.svg" alt="Pixel art character" width="280" />
    </td>
  </tr>
</table>

---

<h2 align="center">Connect</h2>

<p align="center">
  <a href="https://github.com/KOKONOI1706">
    <img src="https://skillicons.dev/icons?i=github" width="42" alt="GitHub" />
  </a>
  <a href="https://www.linkedin.com/in/công-nguyễn-duy-b939b3299">
    <img src="https://skillicons.dev/icons?i=linkedin" width="42" alt="LinkedIn" />
  </a>
  <a href="mailto:nguyenduycongtm@gmail.com">
    <img src="https://skillicons.dev/icons?i=gmail" width="42" alt="Email" />
  </a>
</p>

<h2 align="center">Tech Stack</h2>

<p align="center">
  <img src="https://skillicons.dev/icons?i=java,spring,nodejs,react,nextjs,ts,python,postgres,mysql,mongodb,docker,git,github,linux" alt="Technology stack" />
</p>

<p align="center">
  <sub>Java · Spring Boot · Node.js · React · Next.js · TypeScript · Python · PostgreSQL · MySQL · SQL Server · Supabase · Elasticsearch</sub>
</p>

---

<h2 align="center">Featured Work</h2>

<table align="center">
  <tr>
    <td width="50%" valign="top">
      <h3>EV Charging Station Management System</h3>
      <p>Full-stack platform covering charging stations, reservations, payments, authentication, and operational management.</p>
      <p><strong>Team Lead · System Design · Backend · Frontend · Database</strong></p>
      <a href="https://github.com/KOKONOI1706/EV-Charging-Station-Management-System-BE">View project →</a>
    </td>
    <td width="50%" valign="top">
      <h3>FU Charging Marketplace</h3>
      <p>University marketplace concept with identity, moderation, reservation, and deposit workflows for peer-to-peer trading.</p>
      <p><strong>Business Rules · Requirements · System Design · Full-stack</strong></p>
      <a href="https://github.com/KOKONOI1706/fu-swap-spot">View project →</a>
    </td>
  </tr>
</table>

---

<h2 align="center">GitHub Stats</h2>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=KOKONOI1706&show_icons=true&hide_border=true&rank_icon=github&include_all_commits=true&count_private=true" alt="GitHub statistics" height="165" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=KOKONOI1706&layout=compact&hide_border=true&langs_count=8" alt="Top languages" height="165" />
</p>

<h2 align="center">Activity</h2>

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=KOKONOI1706&hide_border=true&area=true" alt="Contribution activity graph" width="90%" />
</p>

<p align="center">
  <strong>Software Engineer → System Designer → Development Lead</strong>
</p>
